...
- [x] **RagDeepDive** reranker section (~L435-443): Cohere Rerank pricing stale, "Claude/GPT-4" as LLM-reranker. Refresh to Rerank 3.5, bge-reranker-v2-m3, ColBERT-style late interaction. *(2026-09-23: rewrote the "Which reranker?" Decision into four availability-checked pills — hosted cross-encoders (Cohere Rerank 3.5, Voyage rerank-2.5, Jina v2) priced per search rather than a stale hardcoded rate, open-weights (bge-reranker-v2-m3, Qwen3-Reranker) with the fine-tuning argument, ColBERT-style late interaction with the index-size tradeoff, and LLM-as-reranker retiered away from named 2024 models with a don't-use-a-reasoning-model note.)*
- [x] **FineTuningVsRag** — stalest post overall: "student can't exceed teacher" (~L554) is wrong in 2026 (distillation + RL beats teacher on narrow tasks); A100-era hardware; GPT-4o-mini pricing; overstates Anthropic API fine-tuning availability (~L534). No DPO/GRPO coverage. *(2026-09-16: rewrote the LoRA/QLoRA hardware + API-tuning pills to be availability-checked rather than priced, replaced the teacher-ceiling claim with filtered-distillation + what synthetic data still can't do, added an SFT vs DPO vs GRPO Decision. Decision-tree branch split out below.)*
- [x] **FineTuningVsRag** decision tree: no long-context / prompt-caching branch — "just put it in the prompt and cache it" is now a real alternative to both RAG and fine-tuning and the tree never offers it. *(2026-09-26: added a "does the whole corpus fit in the context window?" Decision (fits comfortably → stable cached prefix, 0.1× reads / 1.25×–2× writes, ~30K fresh-equivalent per query for a warm 300K prefix; barely fits → mid-context recall, TTFT and cache-miss risk, eval at your fill level; doesn't fit or per-tenant ACLs → RAG). The SVG tree's knowledge branch now asks "Fits in context?" first and leads to a "Prompt + cache" leaf. Intro and closing Insight updated to match.)*
- [x] **AgentMemory** (~L305): "128K-200K tokens for modern models" — 1M windows ship now. Also L554 pricing. Reconcile the soft-delete (L471) vs GDPR hard-delete (L576) contradiction with one clause on tombstoning vs erasure.
- [x] **AiUxPatterns**: TTFT table names GPT-4o/Claude 3.5/Gemini 1.5/Llama 3 (code L88-92); "GPT-4 hallucinates 3-5%" (L621); hardcoded Anthropic rate limits (L729); per-model timeouts ignore extended thinking (L727). Replace model tables with tier-based rules. *(2026-09-26: TTFT table now budgets by tier × thinking mode with "measure your p50/p95"; hallucination header drops the GPT-4 percentages for grounded-vs-open-ended + measure-on-your-eval; 429 pill explains RPM + input/output TPM per usage tier, `retry-after`, token bucket sized from the console; error-recovery.js derives timeouts from `timeoutFor(tier, thinking effort)` with a thinking-on scenario, and the timeout pill + cardinal-sin Insight recommend idle (no-event) timeouts over per-model constants.)*
