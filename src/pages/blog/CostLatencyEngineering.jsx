...
function Tab5() {
  return (
    <FadeIn>
      ...
      <Decision question="How do you budget an agent loop, where one task is 30 model calls?">
        <p><Pill type="red">Cost grows quadratically, not linearly</Pill> The per-request mental model breaks the moment you ship an agent. Every step re-sends the entire trajectory so far -- system prompt, tool definitions, and all prior tool results. Step 1 sends 2K tokens, step 20 sends 40K. A 30-step task does not cost 30x a single call; it costs closer to <em>n&sup2;/2</em> in input tokens. This is why teams see a $0.004 chat request turn into an $8 agent task and cannot explain the invoice.</p>
        <p><Pill type="amber">Budget the trajectory, not the turn</Pill> max_tokens caps one response and does nothing to stop a loop. You need three independent limits on every agent run: a <strong>step cap</strong> (hard stop at N tool calls), a <strong>cumulative dollar cap</strong> enforced before each step, and a <strong>wall-clock deadline</strong>. Check them in the harness loop, not in the model call -- the model has no idea how much it has already spent. When a cap trips, return partial results with a clear reason rather than silently truncating; a task that stops at step 18 and says so is debuggable, one that stops silently is not.</p>
        <p><Pill type="green">Attack the input side, because that is where the money is</Pill> Prompt caching is worth far more to an agent than to a chatbot: the trajectory prefix is stable and grows monotonically, so cache the system prompt and tool definitions, and re-mark the cache breakpoint as the transcript extends. Then prune what you re-send -- replace verbatim tool output with a compacted summary once it is a few steps old, and drop the raw payload of any tool result the agent has already acted on. Most agent frameworks default to re-sending everything forever, which is the single most expensive default in the ecosystem.</p>
        <p><Pill type="green">Attribute cost to the task, not the call</Pill> Emit a trace ID at the top of the loop and tag every model call with it, so your dashboard reports <em>cost per completed task</em> alongside <em>task success rate</em>. That ratio is the only number that answers the question leadership actually asks: is this agent cheaper than the human it replaced? A 92% success rate at $6/task loses to an 85% success rate at $0.40/task in almost every real business case -- and you cannot see that tradeoff at all if your metrics stop at the request boundary.</p>
      </Decision>
