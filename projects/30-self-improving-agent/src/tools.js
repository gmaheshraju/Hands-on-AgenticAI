...
export const TOOLS = {
  ...
  wikipedia_article: {
    ...
    execute: async ({ title }) => {
      ...
      const res = await fetch(url, {
        headers: { 'User-Agent': 'SelfImprovingAgent/1.0 (research demo)' },
      ...
      return {
        result: extract,
        metadata: { title: data.title, length: extract.length, source: `https://en.wikipedia.org/wiki/${encodeURIComponent(title)}` },
      };
    },
  },
  ...
  note: {
    description: 'Save a finding to the scratchpad for later reference. Use to organize research by subtopic.',
    parameters: '{ "key": "topic_name", "content": "what you learned" }',
    execute: async ({ key, content }, ctx) => {
      if (!ctx?.scratchpad) return { result: 'Scratchpad not available.', metadata: { error: true } };
      const tokensSaved = ctx.scratchpad.write(key, content, { source: 'agent' });
      return {
        result: `Noted "${key}" (${tokensSaved} tokens parked). Use scratchpad index to recall.`,
        metadata: { key, tokensSaved },
      };
    },
  },

  synthesize: {
    description: 'Synthesize all research into a final comprehensive answer. Call this ONLY when you have enough facts.',
    parameters: '{ "answer": "your complete synthesized answer" }',
    execute: async ({ answer }) => {
      return { result: answer, metadata: { type: 'synthesis', length: answer.length } };
    },
  },
};
...
export async function executeTool(name, input, ctx = {}) {
  ...
  if (!tool) return { result: `Unknown tool: ${name}`, metadata: { error: true, unknown: true } };
